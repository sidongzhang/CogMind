...
### 10.为什么前馈网络要先扩展维度再恢复？
增加模型的表示能力和非线性变换能力
扩展维度（d_model → d_ff）：在更高维度的空间中，模型可以学习更复杂的特征表示
非线性激活：在扩展后的高维空间应用ReLU/GELU，引入非线性变换，让模型能够学习更复杂的函数关系
恢复维度（d_ff → d_model）：保持输入输出维度一致，便于残差连接，确保多个层可以堆叠而不改变维度，相当于"压缩"回原始维度，但保留了变换后的信息

### 11.在什么场景下你会选择GELU而不是ReLU？
```选择GELU的场景：
(1)深度Transformer模型（如BERT、GPT系列）
(2)需要更稳定训练的场景
(3)对性能要求极高的应用
(4)计算资源充足的环境
```
